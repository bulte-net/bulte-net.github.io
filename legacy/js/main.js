...
var flashIt = function(selector){
	$(selector).delay(100).fadeOut().fadeIn('slow');
}

$(document).ready(function ($) {

	window.addEventListener('resize', responsiveIt, false);
	responsiveIt();
	...
	$('#contact_form').on('submit', function(event){
		...
		$.post('/api/message', formStr, function(data){
			alert(data.message);
